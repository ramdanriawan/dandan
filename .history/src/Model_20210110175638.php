        ...
        foreach($models as $index => $model) {
            foreach(parent::getRelation() as $relationName => $relationSetting) 
            {
                ...
                if($relationSetting['relation_name'] == 'belongstoone') {
                    $models[$index]->$relationName = $relation->find($this->{$relationSetting['column']});
                    
                } else if($relationSetting['relation_name'] == 'hastomany') {
                    $models[$index]->$relationName = $relation->where($relationSetting['column'], $model->{$this->primaryKey});
                } else if($relationSetting['relation_name'] == 'hastoone') {
                    $models[$index]->$relationName = $relation->row($relationSetting['column'], $model->{$this->primaryKey});
