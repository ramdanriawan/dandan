...
class Model extends Connection
{
    ...
    public function inject($item)
    {
        ...
        if(isset(parent::getRelations()[$this->table])) {
            foreach(parent::getRelations()[$this->table] as $name => $relation) {
                ...
                if($relation[2] == 'belongstoone') {

                    $this->$name = $this->$name->setTable($relation[0])->setLoop()->setAttributes()->find($this->primaryKey);

                    $only = $this->getAttributes();
                    array_push('loop', $only);

                    foreach($this->$name as $attribute => $attributeValue) {

                        if(!in_array($attribute, $only)) {
                            unset($this->$name->$attribute);
                        }
                    }
                }
            }
